...
export default class Category extends Component {
  //state类似vue的data  初识状态信息没有 调取接口获取 页面更新 
  state = {
    categorys: [], // 所有分类的数组
  ...
  */
  ...
  render() {
    ...
    // Card右上角的结构
    const extra = (
      // onClick的值必须是个函数定义  而不是函数调用 
      // onClick={ this.setState({ showStatus: 1 }) }也就是说 这样写的是错误的  不需要点击 页面render就会调用  
      // 解决方法 使用箭头函数进行包裹
    ...
    return (
      <Card extra={extra}>
        <Table 
          ...
          rowKey="_id"//key
          loading={loading}
          columns={this.columns}//显示列数
          dataSource={categorys}//数据源
          // 设置分页 defaultPageSize默认几条信息  showQuickJumper是否可以快速跳转至某页 Goto
